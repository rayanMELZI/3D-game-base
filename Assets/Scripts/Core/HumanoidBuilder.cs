...
namespace FpsBase
{
    ...
    public struct HumanoidParts
    {
        ...
        public Renderer[] darkTeamRenderers; // hands/feet: darker shade of the team color
    ...
    /// arms and legs, backpack. The whole body is covered by hitboxes: the
    /// CharacterController capsule (torso), plus trigger colliders on the neck
    /// and every limb — and the lethal head trigger. Materials are applied
    /// separately at runtime (ApplyMaterials) so the body can also be baked
    /// into a prefab in the editor.
    /// </summary>
    public static class HumanoidBuilder
    {
        public static HumanoidParts Build(Transform parent, bool addHitboxes)
        {
            ...
            {
                // Lethal head hitbox.
                head.AddComponent<SphereCollider>().isTrigger = true;
                head.AddComponent<Hitbox>().isHead = true;

                // Neck bridge: fills the gap between the controller capsule and the head.
                var neck = new GameObject("NeckHitbox");
                neck.transform.SetParent(t, false);
                neck.transform.localPosition = new Vector3(0, 1.56f, 0);
                var neckBox = neck.AddComponent<BoxCollider>();
                neckBox.isTrigger = true;
                neckBox.size = new Vector3(0.3f, 0.28f, 0.28f);

                // Limb hitboxes (triggers never block movement; they follow the swing).
                foreach (var limb in new[] { armMeshL, armMeshR, legMeshL, legMeshR, footL, footR })
                    limb.AddComponent<BoxCollider>().isTrigger = true;
                handL.AddComponent<SphereCollider>().isTrigger = true;
                handR.AddComponent<SphereCollider>().isTrigger = true;
            }
            ...
            parts.darkTeamRenderers = Renderers(handL, handR, footL, footR);
        ...
        /// Team-colored suit, darker team-shade gloves/boots, dark helmet,
        /// glowing visor + chest stripe.
        /// </summary>
        public static void ApplyMaterials(
            Renderer[] teamRenderers, Renderer[] darkTeamRenderers,
            Renderer headRenderer, Renderer visorRenderer,
            Renderer chestStripeRenderer, Renderer[] allRenderers, Color teamColor)
        {
            var baseMat = EnvironmentBuilder.MakeMaterial(new Color(0.12f, 0.12f, 0.14f), 0.3f, 0.5f);
            ...
            var darkMat = EnvironmentBuilder.MakeMaterial(DarkShade(teamColor), 0.15f, 0.45f);
            foreach (var r in darkTeamRenderers)
                r.material = darkMat;
        ...
        public static void ApplyTeamColor(
            Renderer[] teamRenderers, Renderer[] darkTeamRenderers,
            Renderer chestStripeRenderer, Color teamColor)
        {
            foreach (var r in teamRenderers)
                r.material.color = teamColor;
            foreach (var r in darkTeamRenderers)
                r.material.color = DarkShade(teamColor);
            if (chestStripeRenderer != null)
            {
        ...
        private static Color DarkShade(Color c) => new Color(c.r * 0.45f, c.g * 0.45f, c.b * 0.45f);
